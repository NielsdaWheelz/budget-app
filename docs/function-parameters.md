...
- Do not thread internal replay bookkeeping through domain helper APIs; managed-operation composition carries replay identity structurally via `Flow.call(...)`.
...
- If an external provider requires a bookkeeping name or label, synthesize it inside the adapter layer rather than exposing it in the workspace API.
